#!/bin/bash
# Copyright (C) 2016-2021 Álinson Santos Xavier <dengine@gmail.com>
...
PACKAGE_NAME=org.isoron.habitrack
SDKMANAGER="${ANDROID_HOME}/cmdline-tools/latest/bin/sdkmanager"
VERSION=$(grep versionName habitrack-android/build.gradle.kts | sed -e 's/.*"\([^"]*\)".*/\1/g')
BOOT_TIMEOUT=360
...
core_build() {
    ...
    $GRADLE :habitrack-core:build || fail
}
...
android_test() {
    ...
    if [ -n "$RELEASE" ]; then
        ...
        $ADB install -r ${ANDROID_OUTPUTS_DIR}/apk/release/habitrack-android-release.apk || return 1
    else
        log_info "Installing debug APK..."
        $ADB install -t -r ${ANDROID_OUTPUTS_DIR}/apk/debug/habitrack-android-debug.apk || return 1
    fi
    log_info "Installing test APK..."
    $ADB install -r ${ANDROID_OUTPUTS_DIR}/apk/androidTest/debug/habitrack-android-debug-androidTest.apk || return 1
...
android_build() {
    ...
    rm -vf habitrack-android/build/*.apk

    if [ -n "$RELEASE" ]; then
        log_info "Building release APK..."
        $GRADLE updateTranslators
        $GRADLE :habitrack-android:assembleRelease
        cp -v \
            habitrack-android/build/outputs/apk/release/habitrack-android-release.apk \
            habitrack-android/build/loop-"$VERSION"-release.apk
    fi

    log_info "Building debug APK..."
    $GRADLE :habitrack-android:assembleDebug --stacktrace || fail
    cp -v \
        habitrack-android/build/outputs/apk/debug/habitrack-android-debug.apk \
        habitrack-android/build/loop-"$VERSION"-debug.apk

    log_info "Building instrumentation APK..."
    if [ -n "$RELEASE" ]; then
        $GRADLE :habitrack-android:assembleAndroidTest  \
...
android_accept_images() {
    ...
    rsync -av ${ANDROID_OUTPUTS_DIR}/test-screenshots/ habitrack-android/src/androidTest/assets/
}
...
clean() {
    rm -rfv habitrack-android/.gradle
    rm -rfv habitrack-android/android-pickers/build
    rm -rfv habitrack-android/build
    rm -rfv habitrack-android/habitrack-android/build
    rm -rfv habitrack-core-legacy/.gradle
    rm -rfv habitrack-core-legacy/build
    rm -rfv habitrack-core/.gradle
    rm -rfv habitrack-core/build
    rm -rfv habitrack-server/.gradle
    rm -rfv habitrack-server/build
    rm -rfv habitrack-web/build
    rm -rfv habitrack-web/node_modules
    rm -rfv habitrack-web/node_modules/core-js/build
    rm -rfv habitrack-web/node_modules/upath/build
    rm -rfv .gradle
}
